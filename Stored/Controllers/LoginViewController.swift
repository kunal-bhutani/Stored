...
class LoginViewController: UIViewController {
        ...
        FirebaseAuth.Auth.auth().signIn(withEmail: email, password: password, completion: { [weak self] authResult, error in
            ...
            if !user.isEmailVerified {
                user.sendEmailVerification { error in
                    if let error = error {
                        strongSelf.showAlert(message: "Failed to send verification email. \(error.localizedDescription)")
                    } else {
                        strongSelf.showAlert(message: "A verification email has been sent. Please verify your email and try again.")
                    }
                }
                return
            }
            
            // If the email is verified, fetch user and household data
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "JoinCreateSegue" {
            if let user = sender as? User,
               let destinationVC = segue.destination as? JoinOrCreateHouseholdViewController {
